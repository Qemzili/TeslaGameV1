...
const NesneYenileMurad_556 = (obj, f) => {
	let index = 0
	for (let key in obj) {
		f(key, obj[key], index)
		index++
...
const dbChatAlMurad_556 = chatId => {  // CHAT ID ALMASI
	let data = {
		isPlaying: true,
		members: {}
	}
...
const dbUserAlMurad_556 = firstName => {  // KULLANICI ADI, PUAN ALMASI
	return {
		firstName: firstName,
		isPlaying: true,
		answer: null,
...
const RaundMesajMurad_556 = (chatId, round, time) => {
	let chat = getChat(chatId)
	let answers = []
	NesneYenileMurad_556(chat.members, (memberId, member, memberIndex) => {
	...
	answers = answers.sort((a, b) => oyunDurumuHusnuEhedov[chatId].answersOrder.indexOf(a.memberId) - oyunDurumuMurad_556[chatId].answersOrder.indexOf(b.memberId))
	...
	let startRound = async round => {
		...
		gameState.timeouts.round = setTimeout(() => {
			...
			if (round === process.env.RAUND_SAYI - 1) {
				gameState.timeouts.OyunDurdurMurad_556 = setTimeout(() => {
					OyunDurdurMurad_556(ctx, chatId)
				}, 1000)
			}
			else {
    ...
    await ctx.replyWithMarkdown(ozelMesaj(ctx.update.message.chat.id < 0),{
        reply_markup:{
            inline_keyboard:[
                ...
                [{text:'Resmi Kanalımız 📣', url:`t.me/Murad_556`},{text:'VİP Gruplar 💎', callback_data:'vip'}]
            ]
        }
    })
})
...
bot.action('AZ', ctx=>{
    ...
    ctx.replyWithMarkdown(`*🇦🇿 VİP Gruplar 🏆*`,{
        reply_markup:{
            inline_keyboard:[
                [{text:'1) Qrup ', url:'t.me/Murad_556'}],
                [{text:'2) Qrup ', url:'t.me/Murad_556'}],
...
*🇹🇷 VİP Gruplar 🏆*
        ...
        reply_markup:{
            inline_keyboard:[
                [{text:'1) Grub', url:'t.me/Murad_556'}],
                [{text:'2) Grub', url:'t.me/Murad_556'}],
                [{text:'🔙 Geri', callback_data:'vip'}]
            ]
        }
    })
	...
	if (message.chat.id < 0) {
		...
		if (
			...
			telegram.editMessageCaption(
				...
				null,
				RaundMesajMurad_556(chatId, oyunDurumuMurad_556[chatId].currentRound, oyunDurumuHusnuEhedov[chatId].currentTime),
				{
					parse_mode: "Markdown"
				}
			)
